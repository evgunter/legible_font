...
fn test_embed() {  // this runs a forward pass of CLIP on 3 images, so is very slow (~1min on my CPU)
    ...
    // check that the distances img1-img2 and img1_modified-img2 differ by at most 10%
    // (this should be true even if img1 and img1_modified are not that close to each other:
    //  they should both be nearly perpendicular to img2)
    assert!(dist_img1_img2 < dist_img1_modified_img2 * 1.1);
    assert!(dist_img1_img2 > dist_img1_modified_img2 / 1.1);
}
